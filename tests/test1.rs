...
fn set_internal_serial_number() {
    ...
    CFRH320U93::open()
        .unwrap()
        .set_internal_serial_number(&ser)
        .unwrap();
}
...
fn device_info() {
    println!(
        "Manufacturer: {}",
        CFRH320U93::open().unwrap().manufacturer().unwrap()
    );
    println!(
        "Product String: {}",
        CFRH320U93::open().unwrap().product_string().unwrap()
    );
    println!(
        "Serial Number: {}",
        CFRH320U93::open().unwrap().serial_number().unwrap()
    );
}
...
#[test]
fn iso15693_read() {
    let result = CFRH320U93::open()
        .unwrap()
        .iso15693_read(AccessFlag::WithoutUID, 0x00, 0x0a)
        .unwrap();
    print_vec(&result);
}
...
fn iso15693_write() {
    let array = [
        ...
        0x39, 0x00, 0x00, 0x00, 0x87, 0x93, 0x52, 0x55, 0x32, 0x39, 0x33,
    ];
    CFRH320U93::open()
        .unwrap()
        .iso15693_write(AccessFlag::WithoutUID, 0x00, &array.to_vec())
        .unwrap();
}
...
fn test_write() {
    ...
    CFRH320U93::open()
        .unwrap()
        .iso15693_write(AccessFlag::WithoutUID, 0x00, &erasor)
        .unwrap();
...
fn erase_card() {
    let result = CFRH320U93::open()
        .unwrap()
        .iso15693_read(AccessFlag::WithoutUID, 0x00, 0x10)
        .unwrap();
    let u = useful_bytes(result.as_slice()).len();
    let mut erasor = vec![];
    for _ in 0..u {
...
fn stay_quiet() {
    ...
    CFRH320U93::open()
        .unwrap()
        .iso15693_stay_quiet(AccessFlag::WithoutUID, &uid)
        .unwrap();
}
...
fn select() {
    ...
    CFRH320U93::open()
        .unwrap()
        .iso15693_select(AccessFlag::WithUID, &uid2)
        .unwrap();
}
